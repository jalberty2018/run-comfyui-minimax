# One Click - MiniMax H3 FL2VA with uncensored Qwen-VL tail

Generate video with native audio from text, a still image, or first and last frames. Models, VAEs, LoRAs, custom nodes and workflows are provisioned automatically.
...
This template downloads FL2VA and the tail components, not Ref2VA or a separate Qwen LLM. This avoids unnecessary downloads, storage use and provisioning time. If you do not use prompt enhancement, this tail version is still the smallest download: use a standard workflow and leave the tail unused.
...
| [Uncensored Qwen3.8 + `llama.cpp`](https://console.runpod.io/hub/template/a1nkufhzxq?ref=se4tkc5o) | Fast local prompt generation | Larger download and more RAM |
...
1. [Deploy this FL2VA + tail template](https://console.runpod.io/deploy?template=v7b5g03csk&ref=se4tkc5o).
...
| Blackwell high VRAM | RTX PRO 6000 96 GB | Full MXFP8 | 70 GB | 2 MP, 15 s, 24 fps |
